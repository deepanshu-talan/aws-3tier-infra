...
output "application_url" {
  ...
  value       = var.app_domain_name != "" ? "https://${var.app_domain_name}" : "http://${module.alb.alb_dns_name}"
}

output "application_dns_name" {
  description = "Application DNS record name when Route 53 is configured"
  value       = var.app_domain_name != "" ? var.app_domain_name : null
}

output "alert_topic_arn" {
  description = "SNS topic ARN for CloudWatch alarms"
  value       = var.alarm_notification_email != "" ? aws_sns_topic.alerts[0].arn : null
}
...
output "helpful_commands" {
  ...
  value       = <<-EOT
    ...
    Application URL:
       ${var.app_domain_name != "" ? "https://${var.app_domain_name}" : "http://${module.alb.alb_dns_name}"}
    
    EC2 Access:
       Use AWS Systems Manager Session Manager for normal private EC2 access.
       Bastion enabled: ${var.enable_bastion}
