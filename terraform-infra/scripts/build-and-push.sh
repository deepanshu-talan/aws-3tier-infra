...
IMAGE_TAG="$(git -C "$PROJECT_ROOT" rev-parse HEAD 2>/dev/null || true)"
...
if [ $# -eq 2 ]; then
    ...
    if [ -z "$IMAGE_TAG" ]; then
        print_error "Unable to determine git commit SHA for image tag"
        exit 1
    fi
    FRONTEND_IMAGE="$DOCKERHUB_USERNAME/goal-tracker-frontend:$IMAGE_TAG"
    BACKEND_IMAGE="$DOCKERHUB_USERNAME/goal-tracker-backend:$IMAGE_TAG"
    ...
    if [ -z "$FRONTEND_IMAGE" ] || [ -z "$BACKEND_IMAGE" ]; then
        ...
        echo "  $0 username/frontend:<git-sha> username/backend:<git-sha>"
        echo ""
        echo "Option 3: Set in terraform.tfvars:"
        echo "  frontend_docker_image = \"username/goal-tracker-frontend:<git-sha>\""
        echo "  backend_docker_image  = \"username/goal-tracker-backend:<git-sha>\""
        exit 1
    fi
fi

if [[ "$FRONTEND_IMAGE" == *":latest" ]] || [[ "$BACKEND_IMAGE" == *":latest" ]]; then
    print_error "The mutable 'latest' tag is not allowed. Use immutable git SHA image tags."
    exit 1
fi

if [[ "$FRONTEND_IMAGE" != *:* ]] || [[ "$BACKEND_IMAGE" != *:* ]]; then
    print_error "Image tags are required and must be immutable (for example, git SHA)."
    exit 1
fi
...
if [ -z "$IMAGE_TAG" ]; then
    print_error "Unable to determine git commit SHA for build tag"
    exit 1
fi

if ! docker build -t "goal-tracker-frontend:$IMAGE_TAG" .; then
...
docker tag "goal-tracker-frontend:$IMAGE_TAG" "$FRONTEND_IMAGE"
...
if ! docker build -t "goal-tracker-backend:$IMAGE_TAG" .; then
...
docker tag "goal-tracker-backend:$IMAGE_TAG" "$BACKEND_IMAGE"
